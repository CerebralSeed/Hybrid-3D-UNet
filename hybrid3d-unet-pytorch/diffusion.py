...
class Trainer(object):
    ...
    def save(self, milestone):
        ...
        post_script=self.tag if exists(self.tag) else ''
        torch.save(data, str(self.results_folder / f'model-{milestone}')+post_script+'.pt')
            ...
            while self.step < self.train_num_steps:
                ...
                with torch.no_grad():
                    ...
                    for p in self.model.parameters():
                        ...
                        if torch.any(torch.isinf(p.data)):
                            print("inf found!!")
                        p.data[torch.isnan(p.data)] = 0.0000000001
                        p.data[torch.isinf(p.data)] = 0.0
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        utils.save_image(all_images, str(self.results_folder / f'sample-{milestone}')+str('-loss-')+str(self.sas_loss / self.save_and_sample_every)+str('.png'), nrow = int(math.sqrt(self.num_samples)))
                        self.save(milestone)
                        print("Epoch loss: ", self.sas_loss / self.save_and_sample_every)


                        analysis = pd.DataFrame({'epoch':[str(epoch)],'loss':[str(self.sas_loss/self.save_and_sample_every)]})
                        csv_path=os.path.join(self.results_folder,'model-test-results.csv' )
                        if not os.path.exists('model-test-results.csv'):
                            analysis.to_csv(csv_path, mode='a', header=True)
                        else:
                            analysis.to_csv(csv_path, mode='a', header=False)
                        epoch+=1
                        self.sas_loss = 0.
